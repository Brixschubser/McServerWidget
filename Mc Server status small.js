...
function serverOnline(w) {
  ...
  title.font = Font.heavyRoundedSystemFont(18);

  let version = w.addText("V: " + res.version);
  version.font = Font.blackRoundedSystemFont(13);
  ...
  players.font = Font.blackRoundedSystemFont(12);
  if (res.players.online != 0) {
    let playerLs = w.addText(res.players.list.join(", "));
    playerLs.font = Font.lightRoundedSystemFont(12);
  }

  w.addSpacer();

  let date = w.addText(addDateTime());
  date.font = Font.mediumMonospacedSystemFont(7);
